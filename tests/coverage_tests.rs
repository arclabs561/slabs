            ...
            for window in slabs.windows(2) {
                ...
                if second.start < first.end {
                    ...
                    assert!(
                        ...
                        actual_overlap, overlap, first.start, first.end, second.start, second.end
                    );
                }
            }
        }
...
fn fixed_chunker_no_overlap_means_contiguous() {
    ...
    for window in slabs.windows(2) {
        ...
        assert!(
            gap == 0,
            "Gap of {} between chunks with zero overlap",
            gap
        );
    }
}
        ...
        for (i, slab) in slabs.iter().enumerate() {
            ...
            if slab.text.len() > size * 2 {
                panic!(
                    ...
                    i, slab.text.len(), size, &slab.text[..slab.text.len().min(30)]
                );
            }
        }
    }
